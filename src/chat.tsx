...
const key = getLocalStorage("openaiKey");
...
function App() {
  ...
  >([]);
  const openaiRef: any = useRef(null);
  ...
  const handleChangeKey = useDebounce<string>((value: string) => {
    setOpenaiKey(value);
  }, 300);
  ...
  async function main() {
    ...
    if (key || openaiRef.current) {
      try {
        flag.current = true;
        const stream = await openaiRef.current.chat.completions.create({
          model: "gpt-3.5-turbo-1106",
          messages: error
            ? [{ role: "user", content: chatValue }]
            : [
                ...getLastFourElements(content),
                { role: "user", content: chatValue },
              ],
          stream: true,
        });
        setContent((prev) => {
          return [...prev, { role: "user", content: chatValue }];
        });
        setError(false);
        setChatValue("");
          ...
          if (chunk.choices[0]?.delta?.content) {
            if (!firstChunkReceived.current) {
              ...
              setContent((prev) => {
                ...
                newData[newData.length - 1].content +=
        ...
        if (!firstChunkReceived.current) {
          ...
          setContent((prev) => {
            const newData = JSON.parse(JSON.stringify(prev));
            newData[newData.length - 1].content = String(error);
            return newData;
          });
        }
        firstChunkReceived.current = false;
  ...
  const handleOk = () => {
    ...
    setLocalStorage("openaiKey", openaiKey);
    setOpen(false);
  };
    ...
    <div className={style.root}>
      ...
      <div className="content">
        {!(content && content.length > 0) && (
          <div className="start">
            <div className="start-chatgpt-img">
              <svg
                width="32"
                height="32"
                viewBox="0 0 41 41"
              ...
              endAdornment: (
                <InputAdornment position="start">
                  ...
                  <Button
                    // disabled={disabled}
                    style={{
                      backgroundColor: disabled ? "rgba(0,0,0,0.1)" : "black",
                      color: "white",
                      minWidth: "unset",
                      width: 24,
                      height: 24,
